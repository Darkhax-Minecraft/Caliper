    ...
    public void execute (MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if (args.length == 1) {
            ...
            if ("book".equalsIgnoreCase(args[0]) && sender instanceof EntityPlayerMP) {
                ...
                findBadRecipes((Iterable<IRecipe>) ((EntityPlayerMP) sender).getRecipeBook().getRecipes(), sender, false);
            }
            
            else if ("bookall".equalsIgnoreCase(args[0]) && sender instanceof EntityPlayerMP) {
                
                findBadRecipes((Iterable<IRecipe>) ((EntityPlayerMP) sender).getRecipeBook().getRecipes(), sender, true);
            }
            
            else if ("normalall".equalsIgnoreCase(args[0]) && sender instanceof EntityPlayerMP) {
                
                findBadRecipes(ForgeRegistries.RECIPES, sender, true);
            }
            
            else if ("normal".equalsIgnoreCase(args[0]) && sender instanceof EntityPlayerMP) {
                
                findBadRecipes(ForgeRegistries.RECIPES, sender, false);
            }
